...
6. Add the zones, see `adding-zones.md`
```lua
-- Script that displays all trigger zone names
for  i  =  1, #env.mission.triggers.zones  do
	env.info(env.mission.triggers.zones[i].name)
end
```
7. Create scenarios
8. Use the `build_release.py` script to generate the compiled script file, add the script to the triggers.